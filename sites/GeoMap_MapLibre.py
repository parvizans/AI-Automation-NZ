...
CELL_FILE = ROOT / "datasets" / "CCCCell-Site.csv"
...
def band_radius_km(carrier):
    ...
    if "NR900" in carrier or "N09" in carrier or "N900" in carrier:
        return 1.9
    if "N35" in carrier or "NR35" in carrier:
        return 0.95
    if "NR26" in carrier or "N26" in carrier or "N2600" in carrier or "NR2600" in carrier:
        return 0.9
    if "L23" in carrier or "L2300" in carrier:
        return 1.15
    if "L21" in carrier or "L2100" in carrier or "L18" in carrier:
        return 1.35
    if "L07" in carrier or "NR07" in carrier:
        return 2.1
    return 1.25
...
<body>
  ...
  <div class="map-actionbar" id="dtAtollToolbar">
    ...
    <div class="heat-tuning">
      ...
      <label>Coverage <input id="atollOpacity" type="range" min="0" max="100" value="62" title="Prediction opacity" /><b id="atollOpacityValue">62%</b></label>
    </div>
  </div>
  ...
  <div class="panel">
    <div class="brand-lockup">
      ...
      <div>
        <div class="brand-name">AutomationPark GeoAI</div>
        <div class="brand-sub">MapLibre Engineering Lab</div>
      </div>
    </div>
    ...
    <div class="three-grid carrier-grid">
      <button id="carrierAllBtn">All</button>
      <button id="carrierL07Btn" class="active">L07</button>
    ...
    <input id="shellSize" type="range" min="20" max="130" value="40" />
    <div class="hint mini-hint" id="shellSizeText">Shell radius: 40%</div>
    ...
    <div class="section-title area-section">Area Cell Selector</div>
    ...
    <div class="section-title vnr-section">Existing ANR Lines</div>
    ...
    <div class="section-title vnr-section">Auto NCell Suggestions</div>
  ...
  <script>
    ...
    function azimuthDelta(sourceAzimuth, sourceCoord, targetCoord) {
      ...
      const diff = Math.abs(((bearing - planned + 540) % 360) - 180);
      return diff;
    }
    ...
    function normalizedCarrierValue(carrier) {
      const text = String(carrier || '').toUpperCase();
      if (text.includes('N35') || text.includes('NR35')) return 'N35';
      ...
      if (text.includes('L07') || text.includes('NR07')) return 'L07';
      return text || 'Unknown';
    }
    ...
    function dtRangeLegend() {
      ...
      const bins = (dtLegends[dtColorMode] || []).map(row => [...row]);
      const colorKey = dtColorMode;
      for (const feature of dtPoints.features) {
        const color = feature.properties[colorKey];
    ...
    function carrierLabelRank(label) {
      ...
      if (key === 'L07') return 10;
      if (key === 'L18') return 20;
      if (key === 'L21') return 30;
      if (key === 'L23') return 40;
      if (key === 'N35') return 50;
      if (key === 'NR900') return 60;
      return 90;
    }
    ...
    function drawCarrierShellLabels(width, height) {
      ...
      for (const group of groups.values()) {
        ...
        const labels = group.labels.sort((a, b) => carrierLabelRank(a) - carrierLabelRank(b) || a.localeCompare(b));
    ...
    function carrierExpression() {
      if (carrierFilter !== 'all') {
        const carrier = ['upcase', ['to-string', ['get','CARRIER']]];
        const rules = {
          L07: [['in', 'L07', carrier], ['in', 'NR07', carrier]],
          ...
          N35: [['in', 'N35', carrier], ['in', 'NR35', carrier]],
        };
    ...
    function drawManualNcellOverlay(width, height) {
      ...
      for (const row of rows) {
        const sourceCoordinate = cellCoordinateForName(row.source);
        const targetCoordinate = cellCoordinateForName(row.target);
        if (!sourceCoordinate || !targetCoordinate) continue;
        const source = map.project(sourceCoordinate);
        const target = map.project(targetCoordinate);
        ...
        networkCtx.strokeStyle = 'rgba(248,250,252,0.72)';
        networkCtx.lineWidth = Math.max(2.0, Math.min(3.8, map.getZoom() * 0.22));
        networkCtx.stroke();
        networkCtx.beginPath();
        networkCtx.moveTo(source.x, source.y);
        networkCtx.lineTo(target.x, target.y);
        networkCtx.strokeStyle = row.relationColor || relationColor(row.relation);
        networkCtx.lineWidth = Math.max(1.15, Math.min(2.2, map.getZoom() * 0.14));
        networkCtx.stroke();
        for (const point of [source, target]) {
          networkCtx.beginPath();
          networkCtx.arc(point.x, point.y, Math.max(3.0, Math.min(5.6, map.getZoom() * 0.35)), 0, Math.PI * 2);
          networkCtx.fillStyle = row.relationColor || relationColor(row.relation);
    ...
    function drawExistingAnrOverlay(width, height) {
      ...
      if (!rows.length) return;
      for (const row of rows) {
        const source = map.project(row.sourceCoord);
        const target = map.project(row.targetCoord);
        if (!inLooseViewport(source, width, height, 120) && !inLooseViewport(target, width, height, 120)) continue;
        ...
        networkCtx.lineWidth = Math.max(4.2, Math.min(7.2, map.getZoom() * 0.48));
        networkCtx.stroke();
        networkCtx.beginPath();
        networkCtx.moveTo(source.x, source.y);
        networkCtx.lineTo(target.x, target.y);
        networkCtx.strokeStyle = color;
        networkCtx.lineWidth = Math.max(2.1, Math.min(4.3, map.getZoom() * 0.28));
        networkCtx.stroke();
        for (const point of [source, target]) {
          networkCtx.beginPath();
          networkCtx.arc(point.x, point.y, Math.max(3.8, Math.min(7.2, map.getZoom() * 0.45)), 0, Math.PI * 2);
      ...
      const visibleNeighborSet = new Set(rows.map(row => normalizeCellName(normalizeCellName(row.source) === focusKey ? row.target : row.source)));
      for (const feature of cells.features) {
        const props = feature.properties || {};
        const cellName = props.CELL_NAME || '';
        const isFocus = normalizeCellName(cellName) === focusKey;
        const isNeighbor = visibleNeighborSet.has(normalizeCellName(cellName));
        if (!isFocus && !isNeighbor) continue;
        const point = map.project(feature.geometry.coordinates);
        if (!inLooseViewport(point, width, height, 48)) continue;
        const radius = isFocus
          ? Math.max(13, Math.min(28, map.getZoom() * 1.8))
          : Math.max(10, Math.min(22, map.getZoom() * 1.35));
        networkCtx.beginPath();
        networkCtx.arc(point.x, point.y, radius, 0, Math.PI * 2);
        networkCtx.fillStyle = isFocus ? 'rgba(34,211,238,0.26)' : 'rgba(125,211,252,0.18)';
        networkCtx.strokeStyle = isFocus ? '#22d3ee' : '#7dd3fc';
        networkCtx.lineWidth = isFocus ? 4.2 : 3.0;
        networkCtx.fill();
        networkCtx.stroke();
      }
      networkCtx.globalAlpha = 1;
    ...
    function drawNetworkOverlay() {
      ...
      if (sourceCell || targets.length) {
        ...
        for (const feature of shells.features) {
          const props = feature.properties || {};
          if (props.CELL_NAME === sourceCell) {
            drawShellOverlayFeature(feature, width, height, 'rgba(239,68,68,0.58)', '#ff2d2d', 0.74, 1, Math.max(3.2, Math.min(5.8, map.getZoom() * 0.38)));
          } else if (targetNameSet.has(props.CELL_NAME)) {
            drawShellOverlayFeature(feature, width, height, 'rgba(245,158,11,0.45)', '#fbbf24', 0.62, 1, Math.max(2.6, Math.min(4.8, map.getZoom() * 0.32)));
          }
        }
      }
      ...
      drawSelectedSiteOverlay(width, height);
    ...
    function cellCoordinateForName(cellName) {
      ...
      return shellFeature ? shellTip(shellFeature) : null;
    }
    ...
    function existingAnrVisibleRows() {
      return existingAnrRows.filter(row =>
        row.drawable && (ncellRelationFilter === 'all' || row.relation === ncellRelationFilter)
      );
    }
    ...
    function buildExistingAnrLines() {
      return {
        type:'FeatureCollection',
        features: existingAnrRows
          .filter(row => row.drawable)
          .map((row, index) => ({
            type:'Feature',
            geometry:{ type:'LineString', coordinates:[row.sourceCoord, row.targetCoord] },
            properties:{
              index,
              source:row.source,
              target:row.target,
    ...
    function refreshExistingAnrLayers() {
      ...
      const visibleNeighborNames = [...new Set(visibleRows
        ...
        .filter(Boolean))];
      setLayerVisibility(['existing-anr-lines', 'existing-anr-hit', 'existing-anr-neighbor-sites'], visible);
      setFilterSafe('existing-anr-neighbor-sites', visibleNeighborNames.length ? ['match', ['get','CELL_NAME'], visibleNeighborNames, true, false] : emptyCellNameFilter());
      applyNcellRelationFilter();
      updateExistingAnrUi();
    }
    ...
    function buildManualLines() {
      ...
      for (const row of manualLinkRows()) {
        ...
        features.push({
          type:'Feature',
          geometry:{ type:'LineString', coordinates:[sourceCoordinate, targetCoordinate] },
          properties:{
            source:row.source,
            target:row.target,
            relation:row.relation,
    ...
    function setupMapLayers() {
      ...
      map.addLayer({
        ...
        paint:{
          'line-color':['get','relationColor'],
          'line-width':['interpolate',['linear'],['zoom'],8,1.4,13,2.2,16,3.1],
          'line-opacity':0.94
        }
      });
      map.addLayer({
      ...
      map.addLayer({
        ...
        paint:{
          ...
          'circle-color':'rgba(14,165,233,0.20)',
      ...
      setLayerVisibility(['manual-ncell-lines', 'manual-ncell-hit', 'existing-anr-lines', 'existing-anr-hit', 'existing-anr-neighbor-sites'], ncellVisible);
      ...
      function openNcellLinePopup(e) {
        ...
        new maplibregl.Popup({ closeButton:true, maxWidth:'300px' })
          .setLngLat(e.lngLat)
          .setHTML(`<div class="ap-popup-title">${p.source} &lt;--&gt; ${p.target}</div><div class="ap-popup-row"><b>${p.relation}</b> NCell</div><div class="ap-popup-row">Terrain/LOS panel will attach here in the next build.</div>`)
          .addTo(map);
      }
    ...
    document.getElementById('cleanMapBtn').onclick = () => {
      ...
      siteMarkerScale = 0.8;
      document.getElementById('shellSize').value = 40;
      document.getElementById('shellWidth').value = 65;
      document.getElementById('siteMarkerSize').value = 80;
      document.getElementById('shellSizeText').textContent = 'Shell radius: 40%';
      ...
      setLayerVisibility(['cell-labels', 'manual-ncell-lines', 'manual-ncell-hit', 'existing-anr-lines', 'existing-anr-hit', 'existing-anr-neighbor-sites'], false);
      applyCarrierFilter();
      applyDtViewMode();
      ...
      document.getElementById('carrierAllBtn').classList.remove('active');
      document.getElementById('carrierL07Btn').classList.add('active');
    ...
    document.getElementById('clearViewBtn').onclick = () => {
      ...
      clearExistingAnrFocus();
      setLayerVisibility(['rf-shell-fill', 'rf-shell-outline'], true);
      setLayerVisibility(['dt-heatmap', 'dt-points', 'atoll-raster-layer', 'cell-labels', 'manual-ncell-lines', 'manual-ncell-hit', 'existing-anr-lines', 'existing-anr-hit', 'existing-anr-neighbor-sites'], false);
      document.getElementById('shellsBtn').textContent = 'Shells ON';
      document.getElementById('labelsBtn').textContent = 'Labels OFF';
      document.getElementById('ncellBtn').textContent = 'NCell OFF';
      document.getElementById('shellsBtn').classList.add('active');
...
<div class="ap-hero">
  ...
  <p>New WebGL map engine prototype. Current PyDeck map stays frozen while this one grows feature-by-feature.</p>
</div>
""",
    unsafe_allow_html=True,
)

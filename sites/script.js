...
// ========JS-New-3rd-Tools=======================
// 📊 COVERAGE & CAPACITY PLANNER
// ===============================
function calculateCoverage(){

  let area = parseFloat(document.getElementById("cc_area").value);
  let bandwidth = parseFloat(document.getElementById("cc_bw").value);
  let mimo = parseFloat(document.getElementById("cc_mimo").value);
  let users = parseFloat(document.getElementById("cc_users").value);
  let userTput = parseFloat(document.getElementById("cc_user_tput").value);

  if(isNaN(area) || isNaN(bandwidth) || isNaN(mimo) || isNaN(users)){
    alert("Please fill all fields correctly");
    return;
  }

  // 📡 Total Demand
  let totalDemand = users * userTput; // Mbps

  // 📡 Site Capacity (simplified realistic model)
  let spectralEfficiency = 4; // avg bps/Hz (good 5G condition)
  let siteCapacity = bandwidth * spectralEfficiency * mimo; // Mbps

  // 📡 Sites Required (capacity-based)
  let sitesCapacity = totalDemand / siteCapacity;

  // 📡 Coverage-based estimation (simple assumption)
  let coveragePerSite = 3; // sqkm per site (urban-midband assumption)
  let sitesCoverage = area / coveragePerSite;

  // 📡 Final Sites = worst case
  let finalSites = Math.max(sitesCapacity, sitesCoverage);

  // 📡 Congestion Risk
  let loadPerSite = totalDemand / finalSites;

  let risk;
  if(loadPerSite < siteCapacity * 0.5){
    risk = "Low ✅";
  } else if(loadPerSite < siteCapacity * 0.8){
    risk = "Medium ⚠️";
  } else {
    risk = "High 🔴";
  }
  ...
  // 📡 OUTPUT
  // ===============================
  document.getElementById("cc_sites").innerText =
    "Sites Required: " + Math.ceil(finalSites);

  document.getElementById("cc_capacity").innerText =
    "Capacity per Site: " + siteCapacity.toFixed(0) + " Mbps";
